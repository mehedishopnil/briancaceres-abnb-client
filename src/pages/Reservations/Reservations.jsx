...
const Reservations = () => {
  ...
  const { AllHotelData = [], loading } = useContext(AuthContext);
  ...
  const filterData = (filter) => {
    ...
    if (filter === 'All') {
      ...
      filteredData = AllHotelData.filter((item) => item.category === 'Farms');
    } else {
      // Filter data based on other filters
      filteredData = AllHotelData.filter((item) => item.category === filter);
    }
